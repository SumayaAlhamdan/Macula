...
away_duration = 0
max_focused_duration = 0
status = None
start_time = None

def record_time(new_status):
    global focused_duration, distraction_duration, away_duration, max_focused_duration, status, start_time
    ...
    if new_status != status:
        ...
        if status == 'FOCUSED': 
            ...
            away_duration += current_time - start_time

        # Update the status and start time
        status = new_status
        start_time = current_time

        # Update the database only when there is a change in status
        engagement_data = {
            "studentID": student_id,
            "classroomID": class_id,
            "Engagement Status": status,
            "Focus Duration": focused_duration,
            "Distracted Duration": distraction_duration,
            "Longest Focus Duration": max_focused_duration
        }
        collection.update_one({"studentID": student_id, "classroomID": class_id}, {"$set": engagement_data}, upsert=True)
        print("Status updated to:", status)


# Print durations
print("Focused duration:", focused_duration)
print("Distracted duration:", distraction_duration)
print("Away duration:", away_duration)
print("Max focused duration:", max_focused_duration)
...
# Connect to MongoDB
client = MongoClient('mongodb+srv://nesreengdb:macula2024@macula.svwywkc.mongodb.net/?retryWrites=true&w=majority&appName=Macula')
db = client['macula']
...
PREV_STAT = '' 
COUNTER2 = 0
EYE_threshold = 0.66  # Adjust this value as needed
...
while True:
    ...
    for rect in rects:
        ...
        cv2.putText(frame, "EAR: {:.2f}".format(mar), (800, 20), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
    ...
    ''' engagement_data = {
                    ...
                    "Engagement Status": STATUS,
                    "Focus Duration": 0,
                    "Distracted Duration": 0,
                    "Longest Focus Duration":0
                }
    collection.update_one({"studentID": student_id, "classroomID": class_id}, {"$set": engagement_data}, upsert=True)
    print(STATUS) ''' 
